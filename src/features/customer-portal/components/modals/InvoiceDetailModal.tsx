...
export const InvoiceDetailModal: React.FC<InvoiceDetailModalProps> = ({
  ...
  const handleDownload = () => {
    const text = `INVOICE ${effectiveInvoice.invoiceNumber}\nIssue Date: ${effectiveInvoice.issueDate}\nDue Date: ${effectiveInvoice.dueDate}\nStatus: ${effectiveInvoice.status.toUpperCase()}\nTotal Amount: $${effectiveInvoice.amount.toFixed(2)}\nRemaining: $${effectiveInvoice.amountRemaining.toFixed(2)}`;
    const blob = new Blob([text], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `${effectiveInvoice.invoiceNumber}.txt`;
    a.click();
